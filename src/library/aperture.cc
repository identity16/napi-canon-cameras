...
namespace CameraApi
{
  ...
  EdsInt32 Aperture::ForLabel(const Napi::CallbackInfo &info, const std::string &label)
  {
    try
    {
      double aperture;
      ...
      if (offset != std::string::npos)
      {
        if (suffixOffset != std::string::npos)
        {
          ...
          isOneThird = (label.substr(suffixOffset).find("1/3") != std::string::npos);
        }
        else
        {
      ...
      auto values = isOneThird
                        ? OneThirdApertureValues()
                        : OneHalfApertureValues();
    ...
    throw Napi::Error::New(
        info.Env(), "Label does not match any value");
  }
  ...
  Napi::Value Aperture::ForLabel(const Napi::CallbackInfo &info)
  {
    ...
    try
    {
      auto value = Aperture::ForLabel(info, label);
